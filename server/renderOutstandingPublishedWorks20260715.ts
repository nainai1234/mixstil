...
const REPORT_JSON = path.join(REPORTS_DIR, 'outstanding-published-work-renders-2026-07-15.json');
const REPORT_MD = path.join(REPORTS_DIR, 'outstanding-published-work-renders-2026-07-15.md');
...
try {
  const mixes = await query<any>(
    `select id, title, creator_id, render_status, published_version_id
     from mixes
     where status = 'published' and render_status <> 'ready'
     order by updated_at, id`,
  );
  for (const mix of mixes.rows) {
    let session = sessions.get(mix.creator_id);
    if (!session) {
      session = { id: uid('session_published_render'), token: randomBytes(32).toString('hex') };
      await query(
  ...
  for (const session of sessions.values()) {
    await query('delete from auth_sessions where id = $1', [session.id]);
  }
  await pool.end();
}
...
await writeFile(REPORT_MD, [
  '# Outstanding Published Work Renders',
  '',
