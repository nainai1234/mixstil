...
const reviewPath = path.join(root, 'public/review/content-baseline-batch-016/index.html');
...
assert(manifest.version.includes('batch-016-specific-need-expansion'), 'Manifest must identify batch 016');
for (const inherited of ['012', '013', '014', '015']) {
  assert(manifest.inheritedPromotions.includes(`data/content-baseline/content-baseline-batch-${inherited}-promotion.json`), `Batch 016 must inherit Batch ${inherited} promotion`);
}
assert(manifest.candidates.length === 6, 'Batch 016 must include 6 candidates');
...
assert(counts.sleep === 2 && counts.calm === 2 && counts.focus === 2, 'Batch 016 must include 2 candidates per goal');
assert(new Set(manifest.candidates.map((item) => item.scene)).size === 6, 'Batch 016 scenes must be distinct');
...
for (const candidate of manifest.candidates) {
  ...
  assert(candidate.sources.some((source) => source.path.includes('content-baseline/batch-015')), `${candidate.id} must build from Batch 015 promoted expansions`);
  const support = candidate.sources.filter((source) => !source.path.includes('content-baseline/batch-015'));
...
if (failures.length) {
  throw new Error(`Content baseline audio batch 016 validation failed:\n- ${failures.join('\n- ')}`);
}
