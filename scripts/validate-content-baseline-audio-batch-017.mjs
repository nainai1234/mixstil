...
const reviewPath = path.join(root, 'public/review/content-baseline-batch-017/index.html');
...
assert(manifest.version.includes('batch-017-closing-30-seed-baseline'), 'Manifest must identify batch 017');
for (const inherited of ['012', '013', '014', '015', '016']) {
  assert(manifest.inheritedPromotions.includes(`data/content-baseline/content-baseline-batch-${inherited}-promotion.json`), `Batch 017 must inherit Batch ${inherited} promotion`);
}
assert(manifest.candidates.length === 3, 'Batch 017 must include 3 closing candidates');
...
assert(counts.sleep === 1 && counts.calm === 1 && counts.focus === 1, 'Batch 017 must include 1 candidate per goal');
assert(new Set(manifest.candidates.map((item) => item.scene)).size === 3, 'Batch 017 scenes must be distinct');
...
for (const candidate of manifest.candidates) {
  ...
  assert(candidate.sources.some((source) => source.path.includes('content-baseline/batch-016')), `${candidate.id} must build from Batch 016 promoted expansions`);
  const support = candidate.sources.filter((source) => !source.path.includes('content-baseline/batch-016'));
  assert(support.every((source) => /1-3%|1-2%/.test(source.intendedPerceivedShare)), `${candidate.id} support layers must stay under roughly 3 percent`);
...
if (failures.length) {
  throw new Error(`Content baseline audio batch 017 validation failed:\n- ${failures.join('\n- ')}`);
}
