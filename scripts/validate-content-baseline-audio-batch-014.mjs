...
const reviewPath = path.join(root, 'public/review/content-baseline-batch-014/index.html');
...
assert(manifest.version.includes('batch-014-scene-coverage-expansion'), 'Manifest must identify batch 014');
assert(manifest.inheritedPromotions.includes('data/content-baseline/content-baseline-batch-012-promotion.json'), 'Batch 014 must inherit Batch 012 promotion');
assert(manifest.inheritedPromotions.includes('data/content-baseline/content-baseline-batch-013-promotion.json'), 'Batch 014 must inherit Batch 013 promotion');
assert(manifest.candidates.length === 6, 'Batch 014 must include 6 candidates');
...
assert(counts.sleep === 2 && counts.calm === 2 && counts.focus === 2, 'Batch 014 must include 2 candidates per goal');
assert(new Set(manifest.candidates.map((item) => item.scene)).size === 6, 'Batch 014 scenes must be distinct');
...
for (const candidate of manifest.candidates) {
  ...
  assert(candidate.sources.some((source) => source.path.includes('content-baseline/batch-013')), `${candidate.id} must build from Batch 013 promoted expansions`);
  const support = candidate.sources.filter((source) => !source.path.includes('content-baseline/batch-013'));
...
if (failures.length) {
  throw new Error(`Content baseline audio batch 014 validation failed:\n- ${failures.join('\n- ')}`);
}
